...
MtVertexDescriptor *mtVertexDescNew(void);

MT_EXPORT
MT_API_AVAILABLE(mt_macos(10.11), mt_ios(8.0))
void mtVertexAttrib(MtVertexDescriptor *__restrict vertex, uint32_t attribIndex,
                    MtVertexFormat format, uint32_t offset,
                    uint32_t bufferIndex);

MT_EXPORT
MT_API_AVAILABLE(mt_macos(10.11), mt_ios(8.0))
void mtVertexLayout(MtVertexDescriptor *__restrict vertex, uint32_t layoutIndex,
                    uint32_t stride, uint32_t stepRate,
                    MtVertexStepFunction stepFunction);

MT_EXPORT
MT_API_AVAILABLE(mt_macos(10.11), mt_ios(8.0))
void mtSetVertexDesc(MtRenderPipeline *__restrict pipeline,
